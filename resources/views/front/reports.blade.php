...
@section('title',Lang::get('main.reports_eiti'))
@section('content')
@section('about','active')
@section('repoats','active')
@section('stayles')
...
<main>
    <div class="container mt-5 doc">
        <div>
                ...
                @php
                     $document = \App\LocalValue::getValue($doc->file_tj,$doc->file_ru,$doc->file_en);
                @endphp
                  @php
                      if ($document == "") {
                          continue;
                      }
                      $format=explode('.',$document);
                      $type="doc.svg";
                      switch ($format[1]) {
                          case 'pdf':
                              $type="pdf.svg";
                  ...
                  @endphp
                    <ul class="doc_wrapper d-flex mr-3">
                       ...
                       <li><a style="margin-top:10px" download="{{\App\LocalValue::getValue($doc->title_tj,$doc->title_ru,$doc->title_en)}}" href="/public/uploads/reports/{{\App\LocalValue::getValue($doc->file_tj,$doc->file_ru,$doc->file_en)}}" class="doc_item">{{\App\LocalValue::getValue($doc->title_tj,$doc->title_ru,$doc->title_en)}} </a></li>
                    </ul>
                @endforeach
        </div>
    </div>
