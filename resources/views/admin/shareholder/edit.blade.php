            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-flex align-items-center justify-content-between">
                        <h4 class="mb-0">Редактировать aкционерa</h4>
                        <button type="submit" class="btn btn-primary waves-effect waves-light" data-toggle="modal"
                            data-target="#add_modal_ministry" style="float:right;cursor: pointer; ">
                            <a style="color:cornsilk"
                                href="/admin/organization/{{ $shareholder->organization->id }}/shareholders">Назад</a></button>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form method="POST" action="/admin/shareholder/{{ $shareholder->id }}/update"
                                ...
                                <div class="form-group ">
                                    ...
                                    <input name="fio_ru" type="text" class="form-control"
                                        value="{{ $shareholder->fio_ru }}">
                                </div>
                                <div class="form-group">
                                    <label for="productname">ФИО* (TJ)</label>
                                    <input id="" name="fio_tj" type="text" class="form-control"
                                        value="{{ $shareholder->fio_tj }}">
                                </div>
                                <div class="form-group">
                                    <label for="productname">ФИО* (EN)</label>
                                    <input id="" name="fio_en" type="text" class="form-control"
                                        value="{{ $shareholder->fio_en }}">
                                </div>
                                <div class="row">
                                    <div class="form-group col-lg-6">
                                        <label for="productname">Долия (%)*</label>
                                        <input id="productname" name="stock" type="number" min="1" step="any"
                                            class="form-control" value="{{ $shareholder->stock }}">
                                    </div>
                                    <div class="form-group col-lg-6">
                                    ...
                                    <div class="form-group col-lg-6">
                                        ...
                                        <select class="form-control col-lg-12" id="country_id" name="country_id">
                                            ...
                                            @foreach ($countries as $item)
                                                <option {{ $shareholder->country_id == $item->id ? 'selected' : '' }}
                                                    value="{{ $item->id }}">{{ $item->name_ru }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group col-lg-6">
                                        ...
                                        <select class="form-control" name="type">
                                            <option value="">===============-- Выберите --============= </option>
                                            <option {{$shareholder->type==1 ? 'selected' : ''}} value="1">Государственная</option>
                                            <option {{$shareholder->type==0 ? 'selected' : ''}}  value="0">Частная</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="productname">Биржа ссылка</label>
                                    <input id="productname" name="birja_link" type="text" class="form-control col-lg-12"
                                        value="{{ $shareholder->birja_link }}">
                                </div>
                                        ...
                                        @foreach ($categories as $cat)
                                            <optgroup label="{{ $cat->title_ru }}" style="font-weight:700">
                                                @foreach ($cat->organizations as $org)
                                                    <option {{ $shareholder->organization_id == $org->id ? 'selected' : '' }}
                                                        value="{{ $org->id }}">{{ $org->title_ru }}</option>
                                                @endforeach
                                            </optgroup>
                                        @endforeach
                                    </select>
